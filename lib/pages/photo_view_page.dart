...
// ✨ REMOVED: All manual decoding, resizing, and caching logic is no longer needed.

// Flutter's built-in Image.file and precacheImage handles this much more efficiently.
...
class _PhotoViewPageState extends State<PhotoViewPage>
  ...
  // ✨ MODIFIED: Now uses Flutter's highly optimized `precacheImage`.

  void _preloadImages() {
    // Preload the current image, 2 ahead, and 2 behind.

    for (int i = -2; i <= 2; i++) {
      _precacheImage(_currentIndex + i);
    }
  }

  // ✨ MODIFIED: Helper function to get the file path and precache it.
  ...
  Widget _buildInteractiveView() {
    return PageView.builder(
      ...
      onPageChanged: (index) {
        ...
        // Pre-cache the next and previous images for smooth swiping.

        _precacheImage(index + 2);
  ...
  /// ✨ REWRITTEN: Now uses Image.file for hardware-accelerated performance.
        ...
        child: FutureBuilder<Directory?>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done &&
                snapshot.data != null) {
              final file = File(p.join(snapshot.data!.path, vaultFile.id));
              ...
              return Image.file(
                ...
                // The frameBuilder provides a smooth fade-in animation.
  ...
  void _showInfoSheet() async {
    ...
    // ✨ MODIFIED: Now decodes the raw bytes without resizing for info display.
